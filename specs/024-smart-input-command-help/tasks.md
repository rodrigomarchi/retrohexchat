...
- [x] T010 Add textarea-specific CSS in `apps/retro_hex_chat_web/assets/css/chat.css` — style `.chat-input-area textarea` to match existing input appearance (same font, padding, flex: 1, border), set `resize: none`, `overflow-y: hidden` (will be changed to auto in US3), single-line height by default, ensure 98.css compatibility
...
- [x] T017 [US1] Create syntax tooltip function component in `apps/retro_hex_chat_web/lib/retro_hex_chat_web/components/syntax_tooltip.ex` — render tooltip panel with 98.css sunken border styling, syntax line with parameter highlighting (bold for current param), sub-options list, context message, and detail level variants (beginner: full, expert: syntax only)
...
- [x] T021 [US1] Add tooltip CSS styling in `apps/retro_hex_chat_web/assets/css/chat.css` — style `.syntax-tooltip` container (positioned above input, 98.css sunken panel, z-index below autocomplete dropdown), `.syntax-param` normal and `.syntax-param-active` bold highlighting, `.syntax-suboptions` list, `.syntax-context` message line
...
- [x] T037 [US4] Create HistorySearch component in `apps/retro_hex_chat_web/lib/retro_hex_chat_web/components/history_search.ex` — render inline search bar with 98.css styling (sunken field, label "Pesquisar histórico:"), status text area for "No match" indicator, styled to appear above the input area without overlapping tooltip
...
- [x] T041 [US4] Add CSS for history search bar in `apps/retro_hex_chat_web/assets/css/chat.css` — style `.history-search-bar` (positioned above input, compact height, 98.css sunken panel), `.history-search-input` text field, `.history-no-match` indicator styling
