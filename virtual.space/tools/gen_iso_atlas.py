...
the ``ISO_GEO`` object the sprite atlas needs (per-avatar frame size, columns, and
per-animation/-direction row offsets). The atlas builds each avatar descriptor
from this data, so adding/regenerating a character is a data change — no bespoke
per-avatar code.

Sheet URLs are not printed: the atlas derives them from the roster, and at runtime
the server hands it digested ones.
"""
import glob
import json
import os
