defmodule RetroHexChat.Games.CleanupTask do
  @moduledoc """
  Periodic background task that detects and expires stale game sessions
  ...
  defp do_cleanup do
      ...
      Enum.reduce(stale_sessions, 0, fn session, count ->
        case Registry.lookup(session.token) do
          {:error, :not_found} ->
            ...
            Logger.info("Expired stale game session #{session.token} (status: #{session.status})")
    ...
    if expired_count > 0 do
      Logger.info("Game cleanup: expired #{expired_count} stale sessions")
    end
  ...
  defp cleanup_interval do
    Application.get_env(:retro_hex_chat, :game_cleanup_interval, @default_interval)
  end
end
