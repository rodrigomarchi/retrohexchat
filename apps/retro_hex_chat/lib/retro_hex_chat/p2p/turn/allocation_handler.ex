defmodule RetroHexChat.P2P.Turn.AllocationHandler do
  ...
  alias RetroHexChat.P2P.Turn.Attributes.{ChannelNumber, Data, Lifetime, XORPeerAddress}
  alias RetroHexChat.P2P.Turn.Auth
  alias RetroHexChat.P2P.Turn.Config
  alias RetroHexChat.P2P.Turn.Utils
  ...
  def start_link(args) do
    ...
    GenServer.start_link(
      ...
      name:
        {:via, Registry,
         {RetroHexChat.P2P.Turn.AllocationRegistry, five_tuple, alloc_origin_state}}
    )
  end
  ...
  defp handle_message(%Message{type: %Type{class: :request, method: :refresh}} = msg, state) do
    ...
    with {:ok, key} <- Auth.authenticate(msg, username: state.username, config: state.config),
         {:ok, time_to_expiry} <- Utils.get_lifetime(msg, state.config) do
  ...
  defp handle_message(
    ...
    with {:ok, key} <- Auth.authenticate(msg, username: state.username, config: state.config),
         {:ok, state} <- install_or_refresh_permission(msg, state) do
  ...
  defp handle_message(%Message{type: %Type{class: :request, method: :channel_bind}} = msg, state) do
    ...
    with {:ok, key} <- Auth.authenticate(msg, username: state.username, config: state.config),
      ...
      {:ok, state} = install_or_refresh_permission(msg, state, limit: 1)
  ...
  defp assign_channel(ip_addr, port, number, state) do
    ...
    with true <- number in 0x4000..0x7FFE,
      ...
      state =
        ...
        |> put_in(
          [:chann_to_time, number],
          System.os_time(:second) + state.config.channel_lifetime
        )

      Process.send_after(self(), {:check_channel, number}, state.config.channel_lifetime)
