defmodule RetroHexChat.Bots.Capabilities.Moderation do
  ...
  def description,
    do: dgettext("bots", "Auto-moderation — blocked words, spam/flood detection, caps filter")
  ...
  def default_config do
    %{
      ...
      "caps_min_length" => 10,
      "action" => "warn",
      "warn_message" => dgettext("bots", "Hey {nickname}, please keep it civil."),
  ...
  def validate_config(config) do
    ...
    if action in ["warn", "mute", "kick"] do
      ...
      {:error, dgettext("bots", "action must be one of: warn, mute, kick")}
    end
  end
  ...
  defp check_blocked_words(content, config) do
    ...
    if Enum.any?(blocked, &String.contains?(lower, String.downcase(&1))) do
      {:violation, dgettext("bots", "using blocked words")}
    end
  end
  ...
  defp check_spam(author, content, state, config, now) do
    ...
    if recent_same >= threshold do
      {:violation, dgettext("bots", "spamming (%{count} identical messages)", count: recent_same)}
    end
  end
  ...
  defp check_flood(author, state, config, now) do
    ...
    if recent_count >= threshold do
      {:violation,
       dgettext("bots", "flooding (%{count} messages in %{window}s)",
         count: recent_count,
         window: window
       )}
    end
  ...
  defp check_caps(content, config) do
    ...
    if String.length(letters) >= min_length do
      ...
      if ratio >= threshold do
        {:violation, dgettext("bots", "excessive caps lock")}
      end
    end
  end
  ...
  defp apply_action(state, author, reason, config) do
    ...
    template =
      Map.get(config, "warn_message", dgettext("bots", "Hey {nickname}, please keep it civil."))

    message = String.replace(template, dgettext("bots", "{nickname}"), author)
    message = dgettext("bots", "%{message} (%{reason})", message: message, reason: reason)
