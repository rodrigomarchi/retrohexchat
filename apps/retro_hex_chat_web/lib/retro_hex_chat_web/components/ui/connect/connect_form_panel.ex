defmodule RetroHexChatWeb.Components.UI.ConnectFormPanel do
  @moduledoc """
  Visual body of the connect window: step forms, trusted terminal choices,
  remember-terminal fields and the hidden session POST form.

  This is the portable half of the connect screen — it carries no desktop
  chrome, so the same panel renders inside the `/connect` desktop and inside a
  window on the public landing pages. Every event is addressed through `target`
  so the owning LiveComponent receives it directly.
  """
  ...
  def connect_form_panel(assigns) do
    ~H"""
    <.connect_step
      step={@step}
      nickname={@nickname}
  ...
  defp connect_nickname_step(assigns) do
    ~H"""
    <form phx-submit="connect" phx-target={@target}>
  ...
  defp connect_password_step(assigns) do
    ~H"""
    <form phx-submit="authenticate" phx-target={@target} autocomplete="off">
  ...
  defp connect_register_step(assigns) do
    ~H"""
    <form phx-submit="register" phx-target={@target} autocomplete="off">
