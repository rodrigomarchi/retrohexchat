defmodule RetroHexChatWeb.Components.UI.Accordion do
  @moduledoc """
  ...
  slot :icon, required: true, doc: "16×16 section icon — mandatory for visual consistency"
  slot :inner_block, required: true
  ...
  def accordion_trigger(assigns) do
    ...
    <details name={@group} class="group/accordion peer/accordion" open={@open}>
      <summary class={
        ...
        <Icons.icon_chevron_down class="h-4 w-4 shrink-0 transition-transform duration-200 group-open/accordion:rotate-180" />
      </summary>
    </details>
    """
  end
