defmodule RetroHexChatWeb.Components.UI.ConnectionStatus do
  ...
  defp connection_reconnecting(assigns) do
    ...
    <.alert variant="destructive">
      ...
      <.alert_description>
        {dgettext("chat", "Attempt %{attempt} of %{max_attempts}",
          attempt: @attempt,
          max_attempts: @max_attempts
        )}
      </.alert_description>
  ...
  defp connection_disconnected(assigns) do
    ...
    <.alert variant="destructive">
      <:icon><Icons.icon_close class="w-4 h-4" /></:icon>
      <div class="flex items-center gap-retro-4">
        <.alert_title>{dgettext("chat", "Disconnected")}</.alert_title>
      </div>
      <.alert_description>
        <div class="flex items-center gap-retro-4 mt-retro-4">
          <span>{dgettext("chat", "Connection lost.")}</span>
          <.button
            size="sm"
            variant="outline"
            phx-click={@on_retry}
