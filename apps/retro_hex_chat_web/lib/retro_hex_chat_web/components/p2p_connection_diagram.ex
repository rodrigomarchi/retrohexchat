defmodule RetroHexChatWeb.Components.P2pConnectionDiagram do
  @moduledoc """
  Animated SVG connection diagram showing the bilateral P2P link between peers.
  Renders large browser-specific SVG icons, peer whois info, and an animated
  ...
  defp peer_panel(assigns) do
    ...
    <div class={"p2p-diagram__peer p2p-diagram__peer--#{@side}"}>
      <div class="p2p-diagram__browser-icon">
        <.browser_svg name={@browser_name} />
      </div>
      <div class="p2p-diagram__peer-info">
        <div class="p2p-diagram__peer-header">
          <span class={"p2p-diagram__status-dot #{if @online, do: "p2p-diagram__status-dot--online", else: "p2p-diagram__status-dot--offline"}"}>
          </span>
        ...
        <div class="p2p-diagram__whois">
          <.whois_row
            :if={@info[:browser]}
            icon_type="browser"
            icon_key={@browser_name}
            value={@info[:browser]}
          />
          <.whois_row
            :if={@info[:os]}
            icon_type="os"
            icon_key={extract_os_name(@info[:os])}
            value={@info[:os]}
          />
          <.whois_row :if={@info[:screen]} icon_type="emoji" icon_key="🖥" value={@info[:screen]} />
          <.whois_row
            :if={@info[:language]}
            icon_type="emoji"
            icon_key="🗣"
            value={format_language(@info[:language])}
          />
          <.whois_row
            :if={@info[:timezone]}
            icon_type="emoji"
            icon_key="🕐"
            value={format_timezone(@info[:timezone])}
          />
          <.whois_row
            :if={@info[:cores]}
            icon_type="emoji"
            icon_key="⚙"
            value={"#{@info[:cores]} cores"}
          />
          <.whois_row
            :if={@info[:color_depth]}
            icon_type="emoji"
            icon_key="🎨"
            value={"#{@info[:color_depth]}-bit"}
          />
          <.whois_row :if={@info[:touch] == true} icon_type="emoji" icon_key="👆" value="Touch" />
        </div>
      </div>
    </div>
    """
  ...
  # --- Whois Row (with SVG or emoji icon) ---

  attr :icon_type, :string, required: true
  attr :icon_key, :string, required: true
  ...
  defp whois_row(assigns) do
    ...
    <div class="p2p-diagram__whois-row">
      <span class="p2p-diagram__whois-icon">
        <.whois_icon_svg :if={@icon_type in ["browser", "os"]} type={@icon_type} key={@icon_key} />
        <span :if={@icon_type == "emoji"}>{@icon_key}</span>
      </span>
  ...
  # --- Whois mini SVG icons (12px in rows) ---

  attr :type, :string, required: true
  attr :key, :string, required: true

  defp whois_icon_svg(assigns) do
    ~H"""
    <svg viewBox="0 0 24 24" class="p2p-diagram__whois-svg" aria-hidden="true">
      <path d={icon_path(@type, @key)} fill="currentColor" />
    </svg>
    """
  end

  # --- Large Browser SVG (main icon above each peer) ---

  attr :name, :string, required: true

  defp browser_svg(assigns) do
    ~H"""
    <svg viewBox="0 0 24 24" class="p2p-diagram__browser-svg" aria-hidden="true">
      <path d={browser_icon_path(@name)} fill={browser_color(@name)} />
    </svg>
    """
  end
  ...
  # Globe icon for unknown browsers
  ...
  @spec browser_color(String.t()) :: String.t()
  defp browser_color("chrome"), do: "#4285F4"
  defp browser_color("firefox"), do: "#FF7139"
  defp browser_color("safari"), do: "#006CFF"
  defp browser_color("edge"), do: "#0078D7"
  defp browser_color("opera"), do: "#FF1B2D"
  defp browser_color(_), do: "currentColor"
  ...
  # Simplified Linux penguin (Tux outline)
  ...
  @spec icon_path(String.t(), String.t()) :: String.t()
  defp icon_path("browser", name), do: browser_icon_path(name)
  defp icon_path("os", "windows"), do: @windows_path
  defp icon_path("os", "macos"), do: @apple_path
  defp icon_path("os", "ios"), do: @apple_path
  defp icon_path("os", "android"), do: @android_path
  defp icon_path("os", "linux"), do: @linux_path
  defp icon_path("os", "chromeos"), do: @chrome_path
  defp icon_path(_, _), do: @globe_path

  # ──────────────────────────────────────────────────
  # Name extraction from user-agent strings
  # ──────────────────────────────────────────────────
  ...
  # ──────────────────────────────────────────────────
  # State Derivation (split for low cyclomatic complexity)
  # ──────────────────────────────────────────────────
