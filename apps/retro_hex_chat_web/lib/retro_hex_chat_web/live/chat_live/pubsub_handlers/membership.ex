defmodule RetroHexChatWeb.ChatLive.PubsubHandlers.Membership do
  ...
  def handle_info({:user_left, %{nickname: nick, reason: reason} = payload}, socket) do
    msg =
      if reason do
        dgettext("chat", "%{nickname} has left (%{reason})", nickname: nick, reason: reason)
      else
        dgettext("chat", "%{nickname} has left", nickname: nick)
      end
  ...
  def handle_info({:nick_changed, %{old_nick: old_nick, new_nick: new_nick} = payload}, socket) do
    ...
    msg =
      dgettext("chat", "%{old_nick} is now known as %{new_nick}",
        old_nick: old_nick,
        new_nick: new_nick
      )
    ...
    socket =
      if NotifyList.tracking?(socket.assigns.session.notify_list, old_nick) do
        ...
        |> push_status_message(
          dgettext("chat", "* Your notify list buddy %{old_nick} is now known as %{new_nick}",
            old_nick: old_nick,
            new_nick: new_nick
          ),
          :notify_rename
  ...
  def handle_info({:force_disconnect, %{reason: reason} = payload}, socket) do
    ...
    {:halt,
     ...
     |> Phoenix.LiveView.redirect(
       to:
         Paths.session_clear_path(socket, reason,
