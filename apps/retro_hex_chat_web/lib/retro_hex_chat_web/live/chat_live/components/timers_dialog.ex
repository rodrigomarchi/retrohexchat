defmodule RetroHexChatWeb.ChatLive.Components.TimersDialog do
  @moduledoc """
  Stateful LiveComponent for the Timers window body — the inline-edit-list
  pattern. Mounted inside a server-managed desktop window: presence in the DOM
  means open, so closing the window unmounts the island and reopening starts
  from a blank draft. The running timers map (`timers`, i.e. the parent's
  `user_timers` runtime state) is passed through.

  Owned UI/draft state: `selected`, `editing`, `draft_name`, `draft_repeat`,
  `draft_seconds`, `draft_command`, `error`. Pure-UI events
  (select/add/edit/change/cancel) are component-local; the mutating
  ...
  def mount(socket) do
    {:ok, assign(socket, [{:id, @id}, {:timers, %{}}, {:selected, nil} | blank_draft()])}
  end
