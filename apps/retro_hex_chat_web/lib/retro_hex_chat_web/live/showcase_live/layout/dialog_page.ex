defmodule RetroHexChatWeb.ShowcaseLive.Layout.DialogPage do
  ...
  def mount(_params, _session, socket) do
    {:ok, assign(socket, page_title: dgettext("showcase", "Dialog"), active_page: "dialog")}
  end
  ...
  def render(assigns) do
    ...
    <.showcase_layout active_page={@active_page}>
      <h2 class="text-lg font-bold mb-3">{dgettext("showcase", "Dialog")}</h2>

      <.showcase_card
        title={dgettext("showcase", "Basic Dialog")}
        description="Click the button to open a modal dialog."
      >
        <.button phx-click={show_modal("basic-dialog")}>
          <:icon><Icons.icon_btn_open /></:icon>
          {dgettext("showcase", "Open Dialog")}
        </.button>
        <.dialog id="basic-dialog">
          <.dialog_header id="basic-dialog" title={dgettext("showcase", "Edit Profile")}>
            <:icon><Icons.icon_btn_edit /></:icon>
          </.dialog_header>
          <.dialog_body>
            <.dialog_description class="mb-retro-8">
              {dgettext("showcase", "Make changes to your profile here. Click save when you're done.")}
            </.dialog_description>
            <div class="flex items-center gap-retro-8 mb-retro-4">
              <.label class="text-xs w-16 text-right">{dgettext("showcase", "Name:")}</.label>
              <.input type="text" value="Troll" class="flex-1" />
            </div>
            <div class="flex items-center gap-retro-8">
              <.label class="text-xs w-16 text-right">{dgettext("showcase", "Username:")}</.label>
              <.input type="text" value="@troll" class="flex-1" />
            </div>
          </.dialog_body>
          <.dialog_footer>
      ...
      <.showcase_card
        title={dgettext("showcase", "Confirmation Dialog")}
      ...
      >
        <.button variant="destructive" phx-click={show_modal("confirm-dialog")}>
          ...
          {dgettext("showcase", "Delete Account")}
        </.button>
        <.dialog id="confirm-dialog">
          <.dialog_header id="confirm-dialog" title={dgettext("showcase", "Are you sure?")}>
