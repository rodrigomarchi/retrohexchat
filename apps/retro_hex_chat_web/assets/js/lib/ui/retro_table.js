/**
 * The browser-side controller for the RetroTable component.
 *
 * Knows nothing about LiveView — the hook in hooks/ui/retro_table_hook.js binds
 * this to `this.el` and drives mount/reconcile/destroy. See that file for where
 * it sits; see below for the `data-retro-table-*` contract it reads.
 *
 ...
 * change which rows exist, so those stay server events and arrive back as a
 * fresh header and fresh rows.
 *
 * That split is the reason for `reconcile()`. The server re-renders without
 * knowing that a column is 70px wide, hidden, or that row 12 is selected, so
 * the controller re-asserts all three after every patch — the same arrangement
 * the window manager uses for window geometry.
 *
...
import { repositionMenu } from "./menu";
import {
  MIN_COLUMN_WIDTH,
  distributeWidths,
  nextHiddenColumns,
  columnSignature,
} from "./retro_table_layout";
import { nextRowIndex, nextSelection, pruneSelection, toTSV } from "./retro_table_selection";
...
export function createRetroTable(el, ports = {}) {
  const writeText = ports.writeText || ((text) => navigator.clipboard?.writeText(text));
  const hasClipboard = ports.writeText ? () => true : () => Boolean(navigator.clipboard?.writeText);
  ...
  const table = {
    ...
    forgetColumnsIfReplaced() {
      const signature = columnSignature(this.headings().map((th) => th.dataset.column));
     ...
     */
    measure(headings) {
      const measured = headings.map((th) => th.getBoundingClientRect().width);
      const laidOut = this.grid().getBoundingClientRect().width;
      const widths = distributeWidths(measured, laidOut, MIN_COLUMN_WIDTH);

      widths.forEach((width, index) => {
        this.widths.set(headings[index].dataset.column, width);
      });
    },
    ...
    applySelection() {
      ...
      this.selection = pruneSelection(this.selection, present);
    ...
    toggleColumn(column) {
      const next = nextHiddenColumns(this.hiddenColumns, column, this.visibleHeadings().length);
      // The last column standing names the rows; nextHiddenColumns refuses to hide
      // it and hands back the same set, so there is nothing to re-apply.
      if (next === this.hiddenColumns) return;
    ...
    onKeyDown(e) {
      ...
      const cursorIndex = rows.findIndex((row) => row.dataset.rowId === this.cursorRow);
      const target = nextRowIndex(e.key, cursorIndex, rows.length, this.rowsPerPage(rows));
    ...
    moveTo(rowId, { extend, toggle }) {
      ...
      this.anchorRow = result.anchor;
      this.cursorRow = result.cursor;
      this.applySelection();
    ...
    copySelection() {
      ...
      for (const row of this.rows()) {
        ...
        rows.push(
          columns.map(
            (column) => row.querySelector(`[data-column="${column}"]`)?.textContent.trim() ?? "",
          ),
        );
      }

      const text = toTSV(heading, rows);
      ...
      Promise.resolve(writeText(text)).catch((error) => {
