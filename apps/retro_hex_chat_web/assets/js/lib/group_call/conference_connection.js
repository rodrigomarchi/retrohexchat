/**
 * Conference connection controller (framework-free).
 *
 * Minimal browser-side SFU client owning the raw Phoenix Channel (so SDP/ICE do
 * not travel through the ChatLive process), the RTCPeerConnection, media, tiles,
 * quality, reactions, recovery and stats for a group call. It knows nothing about
 * LiveView: `createConferenceConnection(el, ports)` takes the hook element and a
 * `{ pushEvent }` port, and a test drives it through its public surface
 * (mount/destroy, the server-event methods, the pc/channel events it wires)
 * without a LiveSocket. `hooks/group_call/group_call_webrtc_hook.js` is the thin
 * binding that registers the LiveView events onto it.
 */
...
import { createPeerConnection } from "../p2p/webrtc.js";
import {
  canDeferRecovery,
...
import { focusedTileIndex, isTilePinned, tileIsVisible } from "./layout.js";
import { createTrackRegistry } from "./track_registry.js";
import { createParticipantRegistry } from "./participant_registry.js";
import { createTileView } from "./tile_view.js";
import { localEmptyState, localEmptyStateCopy, participantTileMedia } from "./tiles.js";
import {
  actualMediaState,
  hasLiveTrack,
...
export function createConferenceConnection(el, ports) {
  return {
    mount() {
      this.roomToken = el.dataset.groupCallToken;
      this.joinToken = el.dataset.joinToken;
      ...
      this.participantId = stringOrNull(el.dataset.participantId);
      ...
      this.tileView = createTileView(el, {
      ...
      this.participantQualityEventHandler = (event) => {
        ...
        ports.pushEvent("group_call_participant_quality", payload);
      };
      this.recoveryStateEventHandler = (event) => {
        const payload = event.detail || {};
        if (payload.token && payload.token !== this.roomToken) return;

        ports.pushEvent("group_call_recovery_state", payload);
      };
      ...
      this._bindLocalTile();
      el.addEventListener("group-call:toggle-screen-share", this.toggleScreenShareHandler);
      el.addEventListener("group-call:participant-quality", this.participantQualityEventHandler);
      el.addEventListener("group-call:recovery-state", this.recoveryStateEventHandler);
      ...
      this._applyLayout();
      this._connect();
      ports.pushEvent("group_call_webrtc_ready", {});
    },
    ...
    _connect() {
      ...
      this.channel.on("group_call_offer", (payload) => {
        ports.pushEvent("group_call_offer_received", {
    ...
    _handleJoined(payload = {}) {
      ...
      this._syncLocalTile();
      ports.pushEvent("group_call_client_joined", payload);
    ...
    _attachLocalStream(stream) {
      const video = el.querySelector("[data-group-call-local-video]");
      if (video) {
    ...
    _bindLocalTile() {
      const tile = el.querySelector("[data-group-call-local-tile]");
      if (!tile) return;
    ...
    _toggleTileFocus(tile) {
      ...
      if (isFocused) {
        ports.pushEvent("group_call_clear_focus", {});
      ...
      if (participantId) {
        ports.pushEvent("group_call_focus_participant", {
    ...
    _layoutStateFromDataset() {
      return {
        mode: normalizeLayoutMode(el.dataset.layoutMode),
        focusedParticipantId: stringOrNull(el.dataset.focusedParticipantId),
        focusedStreamId: null,
        selfView: normalizeSelfView(el.dataset.selfView),
        pinnedParticipantIds: idsFromValue(el.dataset.pinnedParticipantIds),
      };
    },

    syncLayoutState(payload) {
    ...
    _applyLayout() {
      ...
      const tiles = Array.from(el.querySelectorAll("[data-group-call-video-tile]"));
      ...
      el.dataset.layoutMode = this.layoutState.mode;
      el.dataset.selfView = this.layoutState.selfView;
      el.dataset.pinnedParticipantIds = (this.layoutState.pinnedParticipantIds || []).join(",");
      if (this.layoutState.focusedParticipantId) {
        el.dataset.focusedParticipantId = this.layoutState.focusedParticipantId;
      } else {
        delete el.dataset.focusedParticipantId;
      }
      ...
      const placeholder = el.querySelector("[data-group-call-remote-placeholder]");
    ...
    _mediaStateFromDataset() {
      return mediaStateFromDataset(el.dataset);
    },

    _devicePreferencesFromDataset() {
      return {
        audioInputId: stringOrNull(el.dataset.audioInputId),
        videoInputId: stringOrNull(el.dataset.videoInputId),
        audioOutputId: stringOrNull(el.dataset.audioOutputId),
      };
    },
    ...
    _sendReaction(reaction) {
      if (!reaction || !this.channel) {
        this._notifyWarning(
          t("Join the conference before sending reactions."),
          "reaction_not_ready",
        );
        return;
      }
    ...
    _applyReaction(payload = {}) {
      ...
      for (const tile of tiles) {
        ...
        const bubble = buildReactionBubble({
          reaction,
          reactionId,
          iconNode: reactionIconNode(el, reaction),
    ...
    async stopScreenShareByModerator(payload = {}) {
      this.screenShareBlocked = payload.server_screen_blocked !== false;
      this._notifyWarning(
        t("Screen sharing was stopped by a moderator."),
        "screen_share_moderated",
      );
    ...
    _pushLocalMediaState(mirrorToLiveView = false) {
      ...
      if (mirrorToLiveView) {
        ports.pushEvent("group_call_media_state_forced", this.mediaEnabled);
      }
    },
    ...
    _handlePushToTalkKeydown(event) {
      ...
      if (!this._hasLocalAudioTrack()) {
        this._notifyWarning(
          t("Microphone is not available for push-to-talk."),
          "ptt_audio_missing",
        );
        return;
      }
    ...
    _handleConnectionStateChange() {
      ...
      ports.pushEvent("group_call_connection_state", { state });
    ...
    _scheduleRecovery(reason, { countAttempt = true } = {}) {
      ...
      const beforeActivity = isDisconnectedReason(reason)
    ...
    retryConnection(trigger = "manual") {
    ...
    _handleRemoteVideoStalled(streamId, reason = "remote_video_stalled") {
      ...
      this.retryConnection("remote_video_stalled");
    },
    ...
    _publishRecoveryState(payload) {
      ports.pushEvent("group_call_recovery_state", {
        attempt: 0,
        max_attempts: this.maxRecoveryAttempts,
        manual_retry: false,
        next_retry_ms: 0,
    ...
    _notifyError(message, code = "connection_failed") {
      ports.pushEvent("group_call_client_error", { code, message });
    },

    _notifyWarning(message, code = "media_warning") {
      ports.pushEvent("group_call_client_warning", { code, message });
    },
    ...
    async _sampleStats() {
      ...
      try {
        ...
        ports.pushEvent("group_call_stats", {
          ...stats,
        ...
        if (participantQuality.participants.length > 0) {
          ...
          ports.pushEvent("group_call_participant_quality", participantQuality);
        }
      } catch (error) {
    ...
    _syncParticipantQualityState(payload) {
      ...
      for (const tile of el.querySelectorAll("[data-group-call-video-tile]")) {
    ...
    _cleanup() {
      ...
      if (this.toggleScreenShareHandler) {
        el.removeEventListener("group-call:toggle-screen-share", this.toggleScreenShareHandler);
      }
      ...
      if (this.recoveryStateEventHandler) {
        el.removeEventListener("group-call:recovery-state", this.recoveryStateEventHandler);
      }
