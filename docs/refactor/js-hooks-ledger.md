...
|---|---|---|---|---|---|---|---|---|
| `group_call/group_call_webrtc_hook.js` | 2414→**37** | sim | sim | sim | W4·W5·W-E·W-H3 | **W-H3: casca real sobre controlador framework-free** | p2p/negotiation, group_call/{quality,payload,reactions,layout,tiles,media_state,track_registry,participant_registry,tile_view}, **group_call/conference_connection** (`createConferenceConnection(el, ports)`, 2037 linhas, dono do Socket/pc/mídia; 0 `this.pushEvent`/`this.el`/`this.handleEvent`/`mounted`) | — (teste em `test/lib/group_call/conference_connection{,_negotiation}.test.js`, 43 casos; os reaches por `channel.on`/`pc` são entradas de integração dirigidas via os métodos do controlador, não lógica presa) |
...
| `p2p/file_transfer_hook.js` | 663→**33** | sim | sim | sim | W6·W-C·W-H4 | **W-H4: casca real sobre controlador framework-free** | p2p/transfer_session (redutor); p2p/file_transfer.{...}; **p2p/file_transfer_controller** (`createFileTransferController(el, ports)`, 573 linhas, dono do DataChannel/sessão/loop de envio; 0 `this.pushEvent`/`this.el`/`this.handleEvent`/`mounted`) | — (teste em `test/lib/p2p/file_transfer_controller.test.js`, 8 casos black-box) |
...
| **E2E pré-existente vermelho na `main`:** `chat-group-call.spec.ts:1275` ("pre-join can enter with microphone and camera disabled") — o `storedPrejoinPreference(audio)` fica `null` em vez de `false`. Falha no HEAD também (baseline confirmado), e é do `group_call_prejoin_hook` (localStorage), não do webrtc hook que refatorei. | `e2e/tests/` | **aberto** — dívida pré-existente do prejoin, tarefa própria |
| **Três módulos de `lib/` fora da leva usam portas de LiveView** (o comando de auditoria do §1 do plano os pega além dos dois hooks de conexão): `lib/p2p/rtc_media_hook_factory.js` (fábrica de hook SANCIONADA — referência-ouro #2 do plano, testada black-box), `lib/window_manager/window_manager.js` e `lib/group_call/prejoin.js` (`this.pushEvent`, anteriores à segunda onda). | `js/lib/**` | **aberto** — fora do escopo da missão dos hooks de conexão; avaliar se são fakes análogos em tarefa própria |
