...
O padrão é o mesmo de `Lobby` e `Arcade`: a facade pública fica em
`RetroHexChat.VirtualSpace`; handlers e LiveViews chamam a facade, não módulos
internos.
...
- server ban: reaproveitar `CheckServerBan` pela pipeline da rota;
- fechar: criador, admin/server operator ou expiração;
- expulsar/mutar/trocar mapa: criador ou admin/server operator;
- rate limit de criação: reaproveitar o rate limiter P2P no começo ou criar uma
  tabela ETS própria `VirtualSpace.RateLimiter`.
...
config :retro_hex_chat,
  ...
  virtual_space_default_max_participants: 20,
...
Um cleanup periódico pode seguir o padrão de `P2P.CleanupTask`.
