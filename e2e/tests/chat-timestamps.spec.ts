 ...
 * @flow S12 [done] Message timestamps use detected browser timezone with the current default `dd/mm HH:MM` format (features P2)
 *
...
test.describe("Message timestamps", () => {
  test("timestamps use detected browser timezone and the default dd/mm HH:MM format (S12)", async ({
    ...
    try {
      ...
      expect(timestampCandidates(utcTimezone, startMs, endMs)).not.toContain(
        actual,
      );
    } finally {
