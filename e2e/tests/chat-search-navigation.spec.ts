 ...
 * @flow S8 [done] Search Next/Prev scrolls the active highlighted result into view and preserves active highlight (features P2)
 *
...
async function expectLocatorInsideMessageViewport(
  chat: ChatPage,
  locator: Locator,
) {
  await expect
    .poll(() =>
      locator.evaluate((el) => {
        ...
        return (
          locatorBox.top >= listBox.top && locatorBox.bottom <= listBox.bottom
        );
      }),
    )
    .toBe(true);
...
test.describe("Search result navigation", () => {
  test("active result scrolls into view and remains highlighted on next/previous (S8)", async ({
    ...
    const fillerLines = Array.from(
      { length: 35 },
      (_, index) =>
        `search-nav-filler-${marker}-${String(index + 1).padStart(2, "0")}`,
    );
