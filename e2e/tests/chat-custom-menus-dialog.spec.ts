...
test.describe("Custom Menus dialog", () => {
  test("validates invalid entries and scopes items to their menu type (U9)", async ({
    ...
    try {
      ...
      await expect(
        owner.chat.customNicklistContextMenuItem(nickLabel),
      ).toBeVisible();
      await expect(
        owner.chat.customNicklistContextMenuItem(channelLabel),
      ).toHaveCount(0);
      await expect(
        owner.chat.customNicklistContextMenuItem(chatLabel),
      ).toHaveCount(0);
      ...
      await expect(
        owner.chat.customConversationContextMenuItem(nickLabel),
      ).toHaveCount(0);
      await expect(
        owner.chat.customConversationContextMenuItem(chatLabel),
      ).toHaveCount(0);
      ...
      await expect(
        owner.chat.customChatContextMenuItem(chatLabel),
      ).toBeVisible();
      await expect(owner.chat.customChatContextMenuItem(nickLabel)).toHaveCount(
        0,
      );
      await expect(
        owner.chat.customChatContextMenuItem(channelLabel),
      ).toHaveCount(0);
    } finally {
